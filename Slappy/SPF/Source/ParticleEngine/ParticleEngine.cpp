...
void CEmitter::AddParticle()
{
	...
	if (iNumParticles < MAX_PARTICLES)
	{
		...
		//use the emitter velocity to manipulate the direction the particle will fly
		addVec4DT(&tParticles[iNumParticles].tVelocity, tParticles[iNumParticles].tVelocity, tVelocity);
...
CParticleEngine *CParticleEngine::GetInstance()
{
	if (!m_pInstance)
	{
		m_pInstance = (CParticleEngine*)MemMalloc(sizeof(CParticleEngine));
...
void CParticleEngine::Init()
{
	...
	for (int i = 0; i < Num_ParticleEffects; i++)
	{
		switch (i)
		{
			...
			case JosieSpawn_ParticleEffect:
				{
					pFile = fopen("ResourceFiles\\ParticleEffects\\JosieSpawn.effect", "rb");
					break;
				}
			case CaptainSallySpawn_ParticleEffect:
				{
					pFile = fopen("ResourceFiles\\ParticleEffects\\CaptainSallySpawn.effect", "rb");
					break;
				}
			case MissElizabethSpawn_ParticleEffect:
				{
					pFile = fopen("ResourceFiles\\ParticleEffects\\MissElizabethSpawn.effect", "rb");
					break;
				}
			case SumiSpawn_ParticleEffect:
				{
					pFile = fopen("ResourceFiles\\ParticleEffects\\SumiSpawn.effect", "rb");
					break;
				}
			...
			case CannonShoot_ParticleEffect:
				{
					pFile = fopen("ResourceFiles\\ParticleEffects\\CannonShoot.effect", "rb");
					break;
				}
		}
...
//	Velocity:		The direction the particles will be shot out at 
//					Pass 0 to ignore this variable and shoot particles in their default direction
//					NOTE***	If you use an attachobject, this variable will be ignored!!!/
...
//	pfRGB:			a pointer to three floats representing the color of the particles to be emitted by this emitter
//
//Purpose:	Provide a way for other modules to add particle effects to the game
//
/////////////////////////////////////////////////////////////////////////*/
void CParticleEngine::CreateEmitter(int iEmitterType, Vec4DT *Position, Vec4DT *Velocity, CBaseObject *pAttachObject, float *pRGB)
{
	//create new particle emitter
	CEmitter *pTemp = (CEmitter*)MemMalloc(sizeof(CEmitter));
	memset(pTemp, 0, sizeof(CEmitter));
	memset(pTemp->tParticles, 0, (MAX_PARTICLES * sizeof(ParticleT)));
	pTemp->CEmitter::CEmitter(&(m_EmitterTemplates[iEmitterType]));
	...
	else
	{
		...
		{
			NEW_VEC4D(tempVec);
			memcpy(&pTemp->tVelocity, &tempVec, sizeof(Vec4DT));
		}
	}
	...
	if (pRGB)
	{
		memcpy(&pTemp->fColor, pRGB, sizeof(float) * 3);
	}
	else
	{
		pTemp->fColor[0] = 1.0f;
